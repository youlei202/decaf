...
def c1_factory_training_jobs(section: Mapping[str, Any]) -> tuple[TrainingJob, ...]:
    """Build 44 factory jobs whose checkpoints feed the 88-state selection."""
    ...
    if len(jobs) != 44:
        raise AssertionError(f"C1 factory plan must contain 44 training jobs, found {len(jobs)}")
    return tuple(jobs)
